...
export function PlayingCard({ card, variant = "board" }: PlayingCardProps) {
  ...
  if (parsedCard.hidden) {
    return (
      ...
      >
        ...
        <div className="absolute inset-0 grid place-items-center text-lg font-semibold tracking-[0.18em] text-white/90">
          HOLD
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className={`absolute left-2 top-2 flex flex-col leading-none ${suit.tone}`}>
        <span className="text-sm font-black">{parsedCard.rank}</span>
        <span className="text-[0.9rem]">{suit.symbol}</span>
      </div>
      <div className={`absolute bottom-2 right-2 rotate-180 flex flex-col leading-none ${suit.tone}`}>
        <span className="text-sm font-black">{parsedCard.rank}</span>
        <span className="text-[0.9rem]">{suit.symbol}</span>
      </div>
      <div className={`grid h-full place-items-center ${suit.tone}`}>
        <span className="text-3xl drop-shadow-sm">{suit.symbol}</span>
      </div>
    </div>
  );
}
