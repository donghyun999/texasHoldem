...
export function PlayingCard({ card, variant = "board" }: PlayingCardProps) {
  ...
  const sizeClass =
    variant === "seat"
      ? "h-15 w-11 rounded-[1rem] sm:h-18 sm:w-14 sm:rounded-2xl"
  ...
  if (parsedCard.hidden) {
    return (
      ...
      >
        ...
        <div className="absolute inset-0 grid place-items-center text-sm font-semibold tracking-[0.14em] text-white/90 sm:text-lg sm:tracking-[0.18em]">
          HOLD
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className={`absolute left-1.5 top-1.5 flex flex-col leading-none sm:left-2 sm:top-2 ${suit.tone}`}>
        <span className="text-xs font-black sm:text-sm">{parsedCard.rank}</span>
        <span className="text-[0.7rem] sm:text-[0.9rem]">{suit.symbol}</span>
      </div>
      <div className={`absolute bottom-1.5 right-1.5 rotate-180 flex flex-col leading-none sm:bottom-2 sm:right-2 ${suit.tone}`}>
        <span className="text-xs font-black sm:text-sm">{parsedCard.rank}</span>
        <span className="text-[0.7rem] sm:text-[0.9rem]">{suit.symbol}</span>
      </div>
      <div className={`grid h-full place-items-center ${suit.tone}`}>
        <span className="text-2xl drop-shadow-sm sm:text-3xl">{suit.symbol}</span>
      </div>
    </div>
  );
}
