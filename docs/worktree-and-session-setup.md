...
- 같은 역할 에이전트 교체는 예외다. 새 task가 시작됐다는 이유만으로 교체하지 않는다.
- 교체가 필요하다고 판단되면 먼저 기존 에이전트 종료, 상태 문서 갱신, 교체 사유 기록을 끝낸다.
- 새 에이전트는 상태 복구를 다시 해야 하므로 시간과 컨텍스트를 더 소모한다. 특별한 문제 없이 반복 생성하지 않는다.
