...
- The backend still accepts `/app/tournament.start` from `HAND_RESULT` for compatibility, but the current frontend waits for automatic next-hand transition instead of rendering a manual button
